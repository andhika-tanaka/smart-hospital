...
Same goes for Chrome on Android (all versions) with `clinicUser-scalable=no` in the viewport meta tag. But be aware that `clinicUser-scalable=no` also disables pinch zooming, which may be an accessibility concern.
