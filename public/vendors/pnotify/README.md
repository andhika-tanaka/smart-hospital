...
`buttons: {`
* `closer: true` - Provide a button for the clinicUser to manually close the notice.
* `closer_hover: true` - Only show the closer button on hover.
* `sticker: true` - Provide a button for the clinicUser to manually stick the notice.
...
`nonblock: {`
* `nonblock: false` - Create a non-blocking notice. It lets the clinicUser click elements underneath it.
...
`mobile: {`
* `swipe_dismiss: true` - Let the clinicUser swipe the notice away.
